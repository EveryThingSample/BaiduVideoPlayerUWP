...
namespace EveryThingSampleTools.UWP.NetdiskOpenAPI
{
    internal static class NetdiskOpenAPI
    {
        ...
        public class OAuthToken
        {
            ...
            /// <returns></returns>
            public static Windows.Foundation.IAsyncOperation<OAuthToken> GetOAuthTokenFromCodeAsync(string code, string apiKey, string secretKey, string redirect_uri = "obb")
            {
                var url = $"https://openapi.baidu.com/oauth/2.0/token?grant_type=authorization_code&code={code}&client_id={apiKey}&client_secret={secretKey}&redirect_uri={redirect_uri}";
        ...
        public class XpanShareClass : NetdiskOpen
        {
            private bool IsInlitialized { get; set; }
            public string ShareUrl { get; }
            public string Surl { get { return ShareUrl?.Length > 1 ? ShareUrl.Substring(1) : null; } }
            public string Pwd { get; }
            public string Uk { get; private set; }
            private string RandSk { get; set; }
            ...
            public XpanShareClass(string privateKey, string uk, string shareid)
            {
                RandSk = privateKey;
                Uk = uk;
                ShareId = shareid;
            ...
            /// 
            /// </summary>
            /// <param name="shareLink">https://pan.baidu.com/s/1</param>
            /// <param name="pwd"></param>
            public XpanShareClass(string shareLink, string pwd)
            {
                var arr = shareLink.Split(new string[] { "/s/" }, StringSplitOptions.None);
                Pwd = pwd;
                ShareUrl = arr[1];
            }
            public async Task<bool> InlitializeAsync()
            {
                if (IsInlitialized)
                    return true;
                var url = "https://pan.baidu.com/api/shorturlinfo?shorturl=" + ShareUrl;
                var res = await RequestAsync<ShareViewClass>(url);

                if (res?.shareid != null && res?.uk != null)
                {
                    this.ShareId = res.shareid;
                    this.Uk = res.uk;
                    if (res.errno != -9 && Pwd == "")
                    {
                        return IsInlitialized = true;
                    }
                    url = $"https://pan.baidu.com/rest/2.0/xpan/share?method=verify&surl={this.Surl}&uk={Uk}";
                    var post = "pwd=" + Pwd;
                    var h = new System.Net.WebHeaderCollection();
                    h[System.Net.HttpRequestHeader.Referer] = "pan.baidu.com";
                    res = await RequestAsync<ShareViewClass>(url, post, h);
                    if (res?.randsk != null)
                    {
                        this.RandSk = res.randsk;
                        return IsInlitialized = true;
                    }
                    return false;
                }
            ...
            public Windows.Foundation.IAsyncOperation<ShareViewClass> GetListAsync(ulong fs_id = 0)
            {
                var url = $"https://pan.baidu.com/rest/2.0/xpan/share?method=list&shareid={ShareId}&uk={Uk}&sekey={RandSk}&root=1";
                if (fs_id != 0)
                {
                    url = $"https://pan.baidu.com/rest/2.0/xpan/share?method=list&shareid={ShareId}&uk={Uk}&sekey={RandSk}&fid={fs_id}";
                }
                return RequestAsync<ShareViewClass>(url);
            }
            public async Task<int> SaveFilesAsync(ulong[] fs_ids, string to_path, string access_token)
            {
                var url = $"https://pan.baidu.com/rest/2.0/xpan/share?method=transfer&shareid={ShareId}&from={Uk}&sekey={RandSk}&access_token={access_token}";
                var post = $"fsidlist=[{string.Join(",", fs_ids)}]&path={UrlEncodeToUpper.UrlEncode(to_path)}";
                var h = new System.Net.WebHeaderCollection();
                h[System.Net.HttpRequestHeader.Referer] = "pan.baidu.com";
                var res = await RequestAsync<ShareViewClass>(url, post, h);
                if (res != null)
                    return res.errno;
                return -1;
            }
            public class ShareViewClass
            {
                public int errno { get; set; }
                public string uk { get; set; }
                public string shareid { get; set; }
                ...
                public IList<NetdiskBasic.FileList.ListItem> list { get; set; }
            }
        }
    }
}